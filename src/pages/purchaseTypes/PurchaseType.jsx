...
function DataTables() {
    ...
    const checkPrivileges = () => {
        ...
        setAllowAdding(privilege.includes('PURCHASE_TYPE_ADD'));
        setAllowUpdating(privilege.includes('PURCHASE_TYPE_EDIT'));
        setAllowDeleting(privilege.includes('PURCHASE_TYPE_DELETE'));
    }
    ...
    const columns = [
        {Header: "Adı", accessor: "purchaseTypeName", width: "20%"},
    ...
    const fetchPurchaseTypeList = async () => {
        setLoading(true);
        try {
            const response = await apiClient.get(`${baseURL}/purchase-type`);
            if (response.status >= 200 && response.status < 300) {
                const data = response.data.data || response.data;
                setPurchaseTypeList(data);
            }
        } catch (error) {
            setLoading(false);
        } finally {
    ...
    useEffect(() => {
        const updatedRows = purchaseTypeList.map((item) => ({
    ...
    const handleEdit = (row) => {
        navigate(
            `/modules/settings/purchase-type/edit/${row.original.purchaseTypeId}`
        );
    };
        ...
        if (selectedClientIdToDelete) {
            try {
                const response = await apiClient.delete(
                    `${baseURL}/purchase-type/${selectedClientIdToDelete}`
                );
                if (response.status >= 200 && response.status < 300) {
                    toast.success("Məlumat müvəffəqiyyətlə silindi" + "!");
                    setDatatableData((prevData) => {
                        const updatedRows = prevData.rows.filter(
                            (i) => i.purchaseTypeId !== selectedClientIdToDelete
                        );
                        return {
                            ...prevData,
                            rows: updatedRows,
    ...
    const renderActions = (row) => {
        const id = row.original?.purchaseTypeId;
                                ...
                                <MDBox display="flex" gap={2} alignItems="center" pr={3}>
                                    {
                                        allowAdding && (
                                            <MDButton
                                                ...
                                                onClick={() =>
                                                    navigate("/modules/settings/purchase-type/new")
                                                }
                                            >
