...
function DataTables() {
    ...
    const checkPrivileges = () => {
        ...
        setAllowAdding(privilege.includes('PRODUCT_CATEGORY_ADD'));
        setAllowUpdating(privilege.includes('PRODUCT_CATEGORY_EDIT'));
        setAllowDeleting(privilege.includes('PRODUCT_CATEGORY_DELETE'));
    }
    ...
    const columns = [
        {Header: "Adı", accessor: "itemCategoryName", width: "20%"},
    ...
    const fetchItemCategoryList = async () => {
        setLoading(true);
        try {
            const response = await apiClient.get(`${baseURL}/item-category`);
    ...
    useEffect(() => {
        const updatedRows = itemCategoryList.map((item) => ({
    ...
    const handleEdit = (row) => {
        navigate(
            `/modules/settings/item-category/edit/${row.original.itemCategoryId}`
        );
    };
        ...
        if (selectedClientIdToDelete) {
            try {
                const response = await apiClient.delete(
                    `${baseURL}/item-category/${selectedClientIdToDelete}`
                );
                if (response.status >= 200 && response.status < 300) {
                    toast.success("Məlumat müvəffəqiyyətlə silindi" + "!");
                    setDatatableData((prevData) => {
                        const updatedRows = prevData.rows.filter(
                            (i) => i.itemCategoryId !== selectedClientIdToDelete
                        );
                        return {
                            ...prevData,
                            rows: updatedRows,
    ...
    const renderActions = (row) => {
        const id = row.original?.itemCategoryId;
                                ...
                                <MDBox display="flex" gap={2} alignItems="center" pr={3}>
                                    {
                                        allowAdding && (
                                            <MDButton
                                                ...
                                                onClick={() =>
                                                    navigate("/modules/settings/item-category/new")
                                                }
                                            >
