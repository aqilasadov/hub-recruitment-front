...
function NewRoom() {
  ...
  useEffect(() => {
    if (isEditMode) {
      const fetchBankById = async () => {
        try {
          const response = await apiClient.get(`${baseURL}/room/${id}`);
  ...
  const validationSchema = Yup.object().shape({
    roomName: Yup.string().required("Otaq adı mütləqdir"),
  });
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (isEditMode) {
        await apiClient.put(`${baseURL}/room`, {
          roomId: values.roomId,
          ...payload,
        });
        toast.success("Uğurla redaktə olundu");
      } else {
        await apiClient.post(`${baseURL}/room`, payload);
        toast.success("Uğurla əlavə olundu");
      }

      navigate("/modules/settings/rooms");
    } catch (error) {
                    ...
                    >
                      <MDBox>
                        <MDTypography variant="h5" fontWeight="medium" mb={1}>
                          {isEditMode ? "Otaq Adını redaktə et" : "Yeni Otaq"}
                        </MDTypography>
                      </MDBox>
                      <MDBox display="flex" gap={2}>
                        <MDButton
                          variant="gradient"
                          color="dark"
                          onClick={() => navigate("/modules/settings/rooms")}
                          sx={{
                            color: "darkBlue.main",
                            background:
