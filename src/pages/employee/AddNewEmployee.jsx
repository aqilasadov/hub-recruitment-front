...
function NewEmployee() {
  ...
  const [education, setEducation] = useState([{ id: Date.now() }]);
  ...
  const fetchPosList = async (id) => {
    try {
      const response = await apiClient.get(
        `${baseURL}/employee-positions/by-dep-id/${id}`
      );
      if (response) {
  ...
  const getStepTouchedFields = (step) => {
    switch (step) {
      ...
      case 1:
        return [
          "empPosition.procedureTypeId",
          "empPosition.depId",
          "empPosition.posId",
          "empPosition.trialPeriodId",
          "empPosition.contractTermId",
          "empPosition.startDate",
          "empPosition.endDate",
        ];
      case 2:
                  ...
                  <Field name="empPosition.depId">
                    {({ field, form, meta }) => (
                      <Autocomplete
                        ...
                        value={
                          depList.find(
                            (item) => item.depId === Number(field.value)
                          ) || null
                        }
                        onChange={async (e, newValue) => {
                          form.setFieldValue(
                            field.name,
                            newValue ? Number(newValue.depId) : ""
                          );
                          if (newValue) {
                  ...
                  <Field name="empPosition.posId">
                    {({ field, form, meta }) => (
                      <Autocomplete
                        ...
                        getOptionLabel={(option) => option.title || ""}
                        value={
                          posList.find((item) => item.id === field.value) ||
                          null
                        }
                        onChange={(e, newValue) => {
                          form.setFieldValue(
