...
function DataTables() {
    ...
    const checkPrivileges = () => {
        ...
        setAllowAdding(privilege.includes('VAT_TYPE_ADD'));
        setAllowUpdating(privilege.includes('VAT_TYPE_EDIT'));
        setAllowDeleting(privilege.includes('VAT_TYPE_DELETE'));
    }
    ...
    const columns = [
        {Header: "Adı", accessor: "vatTypeName", width: "20%"},
    ...
    const fetchVatTypeList = async () => {
        setLoading(true);
        try {
            const response = await apiClient.get(
                `${baseURL}/vatType/vat-type`
            );
    ...
    useEffect(() => {
        const updatedRows = vatTypeList.map((vatType) => ({
            vatTypeId: vatType.vatTypeId,
            vatTypeName: vatType.vatTypeName,
            isActive: vatType.isActive === 1 ? "Active" : "Passive",
            actions: renderActions(vatType),
        }));
    ...
    const handleEdit = (row) => {
        ...
        navigate(`/modules/settings/vat-types/edit/${row.original.vatTypeId}`);
    };
    ...
    const renderActions = (row) => {
        const id = row.original?.vatTypeId;
...

                                    {
                                        allowAdding && (
                                            <MDButton
                                                ...
                                                onClick={() => navigate("/modules/settings/vat-types/new")}
                                            >
