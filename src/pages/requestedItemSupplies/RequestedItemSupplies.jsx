...
function DataTables() {
    ...
    const [itemSuppliesList, setItemSuppliesList] = useState([]);
    ...
    const checkPrivileges = () => {
        ...
        setAllowAdding(privilege.includes('REQUESTED_ITEM_SUPPLIES_ADD'));
        setAllowUpdating(privilege.includes('REQUESTED_ITEM_SUPPLIES_EDIT'));
        setAllowDeleting(privilege.includes('REQUESTED_ITEM_SUPPLIES_DELETE'));
    }
    ...
    const columns = [
        {Header: "Adı", accessor: "itemName", width: "20%"},
    ...
    const fetchRequestedItemList = async () => {
        setLoading(true);
        try {
            const response = await apiClient.get(
                `${baseURL}/requested-item-supplies/requested-item-supplies-type`
            );
            if (response.status >= 200 && response.status < 300) {
                const data = response.data.data || response.data;

                const filteredData = data.filter((item) => item.supplyTypeId === 2);
                setItemSuppliesList(filteredData);
            }
        } catch (error) {
            setLoading(false);
        } finally {
    ...
    useEffect(() => {
        const updatedRows = itemSuppliesList.map((item) => ({
    ...
    const handleEdit = (row) => {
        navigate(
            `/modules/settings/requested-item-supplies/edit/${row.original.requestedItemSuppliesId}`
        );
    };
        ...
        if (selectedClientIdToDelete) {
            try {
                const response = await apiClient.delete(
                    `${baseURL}/requested-item-supplies/${selectedClientIdToDelete}`
                );
                if (response.status >= 200 && response.status < 300) {
                    toast.success("Məlumat müvəffəqiyyətlə silindi" + "!");
                    setDatatableData((prevData) => {
                        const updatedRows = prevData.rows.filter(
                            (i) => i.requestedItemSuppliesId !== selectedClientIdToDelete
                        );
                        return {
                            ...prevData,
                            rows: updatedRows,
    ...
    const renderActions = (row) => {
        const id = row.original?.requestedItemSuppliesId;
                                ...
                                <MDBox display="flex" gap={2} alignItems="center" pr={3}>
                                    {
                                        allowAdding && (
                                            <MDButton
                                                ...
                                                onClick={() =>
                                                    navigate("/modules/settings/requested-item-supplies/new")
                                                }
                                            >
