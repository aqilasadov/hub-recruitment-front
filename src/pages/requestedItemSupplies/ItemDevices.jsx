...
function DataTables() {
    ...
    const checkPrivileges = () => {
        ...
        setAllowAdding(privilege.includes('REQUESTED_ITEM_DEVICES_ADD'));
        setAllowUpdating(privilege.includes('REQUESTED_ITEM_DEVICES_EDIT'));
        setAllowDeleting(privilege.includes('REQUESTED_ITEM_DEVICES_DELETE'));
    }
    ...
    const fetchRequestedItemList = async () => {
        ...
        try {
            ...
            if (response.status >= 200 && response.status < 300) {
                ...
                const filteredData = data.filter((item) => item.supplyTypeId === 1);
    ...
    const handleEdit = (row) => {
        navigate(
            `/modules/settings/requested-item-devices/edit/${row.original.requestedItemSuppliesId}`
        );
    };
                                ...
                                <MDBox display="flex" gap={2} alignItems="center" pr={3}>
                                    {
                                        allowAdding && (
                                            <MDButton
                                                ...
                                                onClick={() =>
                                                    navigate("/modules/settings/requested-item-devices/new")
                                                }
                                            >
